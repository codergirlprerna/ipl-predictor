  ...
  return (
    <div className="min-h-screen py-6 sm:py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        {/* ── Tabs ── */}
        <div className="flex border-b border-white/10 mb-5 sm:mb-6 overflow-x-auto scrollbar-hide">
          {tabs.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`flex items-center gap-1.5 px-3 sm:px-4 py-2.5 sm:py-3
              ...
              <table className="w-full text-sm min-w-[520px]">
                <thead>
                  <tr className="bg-surface-hover text-gray-400 text-xs uppercase tracking-wider">
                    ...
                    <th className="text-center px-2 sm:px-3 py-3">M</th>
                    <th className="text-center px-2 sm:px-3 py-3">W</th>
                    <th className="text-center px-2 sm:px-3 py-3">L</th>
                    <th className="text-center px-2 sm:px-3 py-3">NR</th>
                    <th className="text-center px-2 sm:px-3 py-3">Pts</th>
                    <th className="text-center px-2 sm:px-3 py-3">NRR</th>
                    <th className="text-center px-2 sm:px-3 py-3 hidden lg:table-cell">Form</th>
                    <th className="text-center px-3 py-3 hidden sm:table-cell whitespace-nowrap">Playoff %</th>
                  </tr>
                </thead>
                <tbody>
                  {DUMMY_POINTS_TABLE.map((team, idx) => (
                    <tr key={team.team}
                      ...
                      <td className="px-3 sm:px-4 py-3 text-gray-400 font-mono text-xs">{idx + 1}</td>
                      <td className="px-3 sm:px-4 py-3">
                        <div className="flex items-center gap-2">
                          <TeamBadge team={team.team} size="sm" />
                          <span className="text-white font-medium hidden sm:block text-sm">{team.fullName}</span>
                          <span className="text-white font-medium sm:hidden text-xs">{team.team}</span>
                        </div>
                      </td>
                      <td className="px-2 sm:px-3 py-3 text-center text-gray-300 text-xs sm:text-sm">{team.played}</td>
                      <td className="px-2 sm:px-3 py-3 text-center text-green-400 font-medium text-xs sm:text-sm">{team.won}</td>
                      <td className="px-2 sm:px-3 py-3 text-center text-red-400 text-xs sm:text-sm">{team.lost}</td>
                      <td className="px-2 sm:px-3 py-3 text-center text-gray-400 text-xs sm:text-sm">{team.nr}</td>
                      <td className="px-2 sm:px-3 py-3 text-center">
                        <span className="font-bold text-sm sm:text-base" style={{color:"#ffffff"}}>{team.points}</span>
                      </td>
                      <td className={`px-2 sm:px-3 py-3 text-center font-mono text-xs ${
                ...
                <div className="card overflow-hidden">
                  {matches.map((match, idx) => (
                    <div key={match.id}
                      ...
                      {/* Venue — desktop only */}
                      <div className="hidden md:block text-right flex-shrink-0 max-w-[140px]">
          ...
          <div className="card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[380px]">
                <thead>
                  <tr className="bg-surface-hover text-gray-400 text-xs uppercase tracking-wider">
                    <th className="text-left px-3 sm:px-4 py-3 w-10">Rank</th>
                    <th className="text-left px-3 sm:px-4 py-3">Player</th>
                    <th className="text-center px-2 sm:px-3 py-3">M</th>
                    <th className="text-center px-2 sm:px-3 py-3">Runs</th>
                    <th className="text-center px-2 sm:px-3 py-3 hidden sm:table-cell">Avg</th>
                    <th className="text-center px-2 sm:px-3 py-3 hidden sm:table-cell">SR</th>
                  </tr>
                </thead>
                <tbody>
                  {DUMMY_STATS.orangeCap.map((player, idx) => (
                    <tr key={player.name}
                      ...
                      <td className="px-2 sm:px-3 py-3 text-center text-gray-300 text-xs sm:text-sm">{player.matches}</td>
                      <td className="px-2 sm:px-3 py-3 text-center">
                        <span className="text-orange-400 font-bold text-sm sm:text-base">{player.runs}</span>
                      </td>
                      <td className="px-2 sm:px-3 py-3 text-center text-gray-300 text-xs sm:text-sm hidden sm:table-cell">{player.avg}</td>
                      <td className="px-2 sm:px-3 py-3 text-center text-gray-300 text-xs sm:text-sm hidden sm:table-cell">{player.sr}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <table className="w-full text-sm min-w-[380px]">
                <thead>
                  <tr className="bg-surface-hover text-gray-400 text-xs uppercase tracking-wider">
                    ...
                    <th className="text-left px-3 sm:px-4 py-3">Player</th>
                    <th className="text-center px-2 sm:px-3 py-3">M</th>
                    <th className="text-center px-2 sm:px-3 py-3">Wkts</th>
                    <th className="text-center px-2 sm:px-3 py-3 hidden sm:table-cell">Avg</th>
                    <th className="text-center px-2 sm:px-3 py-3 hidden sm:table-cell">Econ</th>
                  </tr>
                </thead>
                <tbody>
                  {DUMMY_STATS.purpleCap.map((player, idx) => (
                    <tr key={player.name}
                      ...
                      <td className="px-2 sm:px-3 py-3 text-center text-gray-300 text-xs sm:text-sm hidden sm:table-cell">{player.avg}</td>
                      <td className="px-2 sm:px-3 py-3 text-center text-gray-300 text-xs sm:text-sm hidden sm:table-cell">{player.economy}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
