  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden px-4 pt-10 sm:pt-16 pb-10 sm:pb-12">
        ...
        <div className="max-w-4xl mx-auto text-center relative">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link to="/stats?tab=schedule" style={{ color: "#ffffff" }}
              className="btn-secondary text-sm sm:text-base px-6 sm:px-8 py-2.5 sm:py-3 w-full sm:w-auto text-center">
              View Schedule
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-3 gap-2 sm:gap-3">
          {STATS_STRIP.map(({ icon: Icon, label, color }, idx) => (
            <div key={label} className="card p-3 sm:p-4 text-center">
              ...
              <div className={`text-xl sm:text-2xl font-display ${color}`}>{stripValues[idx]}</div>
            ...
            <div ref={sliderRef}
              ...
              {sliderMatches.map((match) => (
                <Link key={match.id} to={`/match/${match.id}`}
                  className="flex-shrink-0 card p-3.5 hover:border-orange-500/40 hover:-translate-y-0.5 transition-all active:scale-[0.97]"
                  style={{ width: "196px", scrollSnapAlign: "start" }}>
                  <div className="flex items-center justify-between gap-1 mb-2.5">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <TeamBadge team={match.team1} size="sm" />
                      <span className="text-white text-xs font-bold truncate">{SHORT[match.team1] || match.team1}</span>
                    </div>
                    <span className="text-gray-600 text-[10px] font-mono flex-shrink-0">vs</span>
                    <div className="flex items-center gap-1.5 min-w-0 justify-end">
                      <span className="text-white text-xs font-bold truncate">{SHORT[match.team2] || match.team2}</span>
              ...
              {/* Full Schedule end-cap → Schedule tab */}
              <Link to="/stats?tab=schedule"
                className="flex-shrink-0 card flex flex-col items-center justify-center gap-2 border-dashed hover:border-orange-500/40 transition-all p-4"
                style={{ width: "120px", scrollSnapAlign: "start" }}>
                <span className="text-2xl">📅</span>
                <p className="text-orange-400 text-xs font-medium text-center leading-tight">Full Schedule</p>
        ...
        <section className="px-4 pb-8 sm:pb-10">
          <div className="max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              ...
              <Link to="/leaderboard" className="text-orange-400 text-xs flex items-center gap-1 hover:underline">
      ...
      <section className="px-4 pb-8 sm:pb-10">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            ...
            {/* → Orange Cap tab */}
            <Link to="/stats?tab=orange" className="text-orange-400 text-xs flex items-center gap-1 hover:underline">
      ...
      <section className="px-4 pb-12 sm:pb-16">
        <div className="max-w-4xl mx-auto">
          <div className="card p-6 sm:p-8 text-center bg-gradient-to-br from-orange-500/10 to-blue-600/10 border border-orange-500/20">
            <div className="text-3xl sm:text-4xl mb-3 sm:mb-4">🏏</div>
            <h2 className="text-xl sm:text-2xl font-display text-white mb-2">Ready to Sense the Game?</h2>
            <p className="text-gray-400 text-xs sm:text-sm mb-5 sm:mb-6 max-w-xs sm:max-w-sm mx-auto leading-relaxed">
              Submit your prediction before every match.
              Climb the leaderboard. Prove you sense cricket better than AI.
            </p>
            <Link to="/register" className="btn-primary inline-flex items-center gap-2" style={{ color: "#ffffff" }}>
